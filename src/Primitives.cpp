...
void Primitives::floodFill(SDL_Surface *surface, int x, int y, Uint32 fillColor)
{
    ...
    while (!pixels.empty())
    {
        ...
        if (px < 0 || px >= surface->w || py < 0 || py >= surface->h)
            continue;
        if (getPixel(surface, px, py) != targetColor)
            continue;
...
SDL_Surface *Primitives::transformSurface(SDL_Surface *src, const Mat3 &m, int newW, int newH)
{
    ...
    for (int y = 0; y < src->h; y++)
    {
        for (int x = 0; x < src->w; x++)
        {
            ...
            float tx, ty;
            m.transformPoint((float) x,(float) y, tx, ty);
...
void Primitives::blitTransformed(SDL_Surface* dest, SDL_Surface* src, const Mat3& m) {
    SDL_Surface* tmp = Primitives::transformSurface(src, m, dest->w, dest->h);

    SDL_LockSurface(dest);
    SDL_LockSurface(tmp);

    // limpa a surface destino
    SDL_FillRect(dest, NULL, SDL_MapRGBA(dest->format, 0,0,0,0));

    for (int y = 0; y < tmp->h; y++) {
        for (int x = 0; x < tmp->w; x++) {
            Uint32 pixel = getPixel(tmp, x, y);
            
            Uint8 r, g, b, a;
            SDL_GetRGBA(pixel, tmp->format, &r, &g, &b, &a);

            // copia somente se NÃO for preto absoluto (ignora alfa)
            if (r != 0 || g != 0 || b != 0) {
    ...
    SDL_FreeSurface(tmp); // aqui pode descartar a temporária
}

void Primitives::copySurface(SDL_Surface* dest, SDL_Surface* src) {
    SDL_LockSurface(dest);
    SDL_LockSurface(src);
